'use client';

import { useRef, useLayoutEffect } from 'react';
import gsap from 'gsap';
import { ScrollTrigger } from 'gsap/ScrollTrigger';
import { ArrowRight } from 'lucide-react';

gsap.registerPlugin(ScrollTrigger);

export default function PortfolioWeddings() {
    const sectionRef = useRef(null);
    const rightImageRef = useRef(null);
    const topLeftImageRef = useRef(null);
    const bottomLeftImageRef = useRef(null);
    const headlineRef = useRef(null);
    const lineRef = useRef(null);

    useLayoutEffect(() => {
        const section = sectionRef.current;
        if (!section) return;

        const ctx = gsap.context(() => {
            const scrollTl = gsap.timeline({
                scrollTrigger: {
                    trigger: section,
                    start: 'top top',
                    end: '+=130%',
                    pin: true,
                    scrub: 0.6,
                },
            });

            // ENTRANCE (0-30%)
            scrollTl.fromTo(
                rightImageRef.current,
                { x: '60vw', opacity: 0 },
                { x: 0, opacity: 1, ease: 'none' },
                0
            );

            scrollTl.fromTo(
                topLeftImageRef.current,
                { x: '-40vw', opacity: 0 },
                { x: 0, opacity: 1, ease: 'none' },
                0
            );

            scrollTl.fromTo(
                bottomLeftImageRef.current,
                { x: '-40vw', opacity: 0 },
                { x: 0, opacity: 1, ease: 'none' },
                0.06
            );

            scrollTl.fromTo(
                headlineRef.current,
                { x: '18vw', opacity: 0 },
                { x: 0, opacity: 1, ease: 'none' },
                0.1
            );

            scrollTl.fromTo(
                lineRef.current,
                { scaleX: 0 },
                { scaleX: 1, transformOrigin: 'left', ease: 'none' },
                0.08
            );

            // EXIT (70-100%)
            scrollTl.fromTo(
                rightImageRef.current,
                { x: 0, opacity: 1 },
                { x: '18vw', opacity: 0, ease: 'power2.in' },
                0.7
            );

            scrollTl.fromTo(
                topLeftImageRef.current,
                { x: 0, opacity: 1 },
                { x: '-14vw', opacity: 0, ease: 'power2.in' },
                0.7
            );

            scrollTl.fromTo(
                bottomLeftImageRef.current,
                { x: 0, opacity: 1 },
                { x: '-14vw', opacity: 0, ease: 'power2.in' },
                0.72
            );

            scrollTl.fromTo(
                headlineRef.current,
                { y: 0, opacity: 1 },
                { y: 16, opacity: 0, ease: 'power2.in' },
                0.7
            );

            scrollTl.fromTo(
                lineRef.current,
                { scaleX: 1 },
                { scaleX: 0, transformOrigin: 'left', ease: 'power2.in' },
                0.75
            );
        }, section);

        return () => ctx.revert();
    }, []);

    return (
        <section
            ref={sectionRef}
            id="portfolio"
            className="section-pinned z-30 flex items-center"
        >
            {/* Top Left Image */}
            <div
                ref={topLeftImageRef}
                className="absolute left-[6vw] top-[-20%] w-[28vw] h-[34vh] image-frame"
            >
                <img
                    src="/wedding_top_05.jpg"
                    alt="Wedding portrait"
                    className="w-full h-full object-cover"
                />
            </div>

            {/* Bottom Left Image */}
            <div
                ref={bottomLeftImageRef}
                className="absolute right-[10vw] top-[10vh] w-[28vw] h-[34vh] image-frame"
            >
                <img
                    src="/wedding_bottom_06.jpg"
                    alt="Wedding candid moment"
                    className="w-full h-full object-cover"
                />
            </div>

            {/* Right Large Image */}
            <div
                ref={rightImageRef}
                className="absolute left-[1vw] top-[1vh] w-[56vw] h-[74vh] image-frame"
            >
                <img
                    src="/wedding_main_07.jpg"
                    alt="Dramatic wedding portrait"
                    className="w-full h-full object-cover"
                />
            </div>

            {/* Amber Line */}
            <div
                ref={lineRef}
                className="absolute left-[6vw] top-[90vh] w-[34vw] amber-line"
            />

            {/* Headline Block */}
            <div
                ref={headlineRef}
                className="absolute left-[58vw] top-[56vh] w-[34vw]"
                style={{ textShadow: '0 4px 30px rgba(0,0,0,0.8)' }}
            >
                <h2 className="headline-lg text-white mb-4">WEDDINGS</h2>
                <p className="text-white/80 text-base leading-relaxed mb-6">
                    From quiet glances to the dance floor—every chapter, preserved.
                </p>
                <button className="flex items-center gap-2 text-amber font-medium hover:gap-4 transition-all duration-300">
                    Explore wedding stories
                    <ArrowRight className="w-4 h-4" />
                </button>
            </div>
        </section>
    );
}
